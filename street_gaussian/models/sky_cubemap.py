...
from street_gaussian.utils.graphics_utils import get_rays_torch
from street_gaussian.utils.general_utils import get_expon_lr_func
...
class SkyCubeMap(nn.Module):
    def __init__(self):
        ...
        if self.cfg.white_background:
            base = torch.ones(6, self.resolution, self.resolution, 3, device='cuda', dtype=torch.float) * (1. - eps)
        else:
            base = torch.zeros(6, self.resolution, self.resolution, 3, device='cuda', dtype=torch.float) + eps
        ...
        if cfg.data.white_background:
            self.sky_color = torch.ones((max_h, max_w, 3), device='cuda', dtype=torch.float)
        else:
            self.sky_color = torch.zeros((max_h, max_w, 3), device='cuda', dtype=torch.float)
    ...
    def forward(self, camera: Camera, acc=None):
        ...
        if cfg.mode == 'train' and sky_mask is not None:
            ...
            # mask = sky_mask[0].to('cuda', non_blocking=True) | ((1 - acc[0]) > 1e-3)
            # mask[:50, :] = True
        ...
        if mask is None:
            sky_color = dr.texture(self.sky_cube_map[None, ...].sigmoid(), rays_d[None, ...], filter_mode='linear', boundary_mode='cube')
            sky_color = sky_color[0].permute(2, 0, 1).clamp(0., 1.)  # [3, H, W]
        else:
            if cfg.mode == 'train':
                if self.cfg.white_background:
                    sky_color = torch.ones((H, W, 3), device='cuda', dtype=torch.float)
                else:
                    sky_color = torch.zeros((H, W, 3), device='cuda', dtype=torch.float)
            else:
            ...
            with torch.set_grad_enabled(not camera.meta['is_novel_view']):
                if rays_d.numel() > 0:
                    sky_color_mask = dr.texture(self.sky_cube_map[None, ...].sigmoid(), rays_d[None, None, ...], filter_mode='linear', boundary_mode='cube')
                else:
                    sky_color_mask = torch.zeros_like(rays_d[None, None, ...])
